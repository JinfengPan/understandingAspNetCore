...
namespace FishTank.ViewComponents
{
    public class AlertViewComponent : ViewComponent
    {
        ...
        public IViewComponentResult Invoke()
        {
            ...
            if (sensorDataService.GetFishMotionPercentage().Value >
                thresholdConfig.FishMotionMax)
                viewModel.Add("Too much fish activity");
            if (sensorDataService.GetFishMotionPercentage().Value <
                thresholdConfig.FishMotionMin)
                viewModel.Add("Looks like we have some dead fish");
            if (sensorDataService.GetLightIntensityLumens().Value >
                thresholdConfig.LightIntensityMax)
                viewModel.Add("Bright light, bright light!");
            if (sensorDataService.GetLightIntensityLumens().Value <
                thresholdConfig.LightIntensityMin)
                viewModel.Add("It's dark out here");

            if (sensorDataService.GetWaterOpacityPercentage().Value >
                thresholdConfig.WaterOpacityMax)
                viewModel.Add("The fish can't see you");
            if (sensorDataService.GetWaterOpacityPercentage().Value <
                thresholdConfig.WaterOpacityMin)
                viewModel.Add("Water too clean");

            if (sensorDataService.GetWaterTemperatureFahrenheit().Value >
                thresholdConfig.WaterTemperatureMax)
